...
namespace Lermos\UMI\payment
	...
	abstract class payment extends umiObjectProxy {
		...
		/**
		 * Search order ID in the callback of payment system.
		 * Regular fields are being ckecked then getOrderId returns
		 * in every payment system
		...
		/**
		 * Searching order ID in the callback parameters.
		 * Return false if a payment system uses predefined params such as
		 * (orderid, orderId, order-id, order_id).
		 * If not redeclare the function.
		 * @return Integer | boolean false
		 */
